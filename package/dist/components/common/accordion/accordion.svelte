<script>import { createAccordion, melt, createSync } from "@melt-ui/svelte";
import { slide } from "svelte/transition";
import { twJoin } from "tailwind-merge";
import { useProps } from "../../../composables/use/props.js";
import { useUI } from "../../../index.js";
import { accordion } from "./accordion.config.js";
import { strify } from "../../../utils/index.js";
const props = useProps("Accordion");
let _class = props.class;
export { _class as class };
export let links = props.links;
export let value = props.value;
export let disabled = props.disabled;
export let multiple = props.multiple;
export let visible = props.visible;
export let override = props.override;
const {
  elements: { content, item, trigger, root },
  helpers: { isSelected },
  states
} = createAccordion({
  defaultValue: "item-1",
  forceVisible: visible,
  multiple,
  disabled
});
const sync = createSync(states);
$: sync.value(value, (v) => value = v);
const { css, classer } = useUI(accordion, _class, override);
</script>

<div class={twJoin(strify(css.root), classer)} {...$root} use:$root.action>
	{#each links as link, i}
		{@const __MELTUI_BUILDER_2__ = $content(link.id)}
		{@const __MELTUI_BUILDER_1__ = $trigger(link.id)}
		{@const __MELTUI_BUILDER_0__ = $item(link.id)}
		<div id={link.id} {...__MELTUI_BUILDER_0__} use:__MELTUI_BUILDER_0__.action class={strify(css.item)}>
			<button
				{...__MELTUI_BUILDER_1__} use:__MELTUI_BUILDER_1__.action
				class={twJoin(strify(css.item.trigger), i !== 0 && strify(css.item.trigger.first))}
			>
				<slot name="trigger" {link}>
					{link.label}
				</slot>
			</button>

			{#if $isSelected(link.id)}
				<div class={strify(css.item.content)} {...__MELTUI_BUILDER_2__} use:__MELTUI_BUILDER_2__.action transition:slide>
					<slot {link}>
						{link.description}
					</slot>
				</div>
			{/if}
		</div>
	{/each}
</div>
