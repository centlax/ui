...
import { stringify, strify, twJoin } from "../../../utils/index.js";
...
{#if $$slots.trigger}
	<div {...$trigger} use:$trigger.action class={stringify(css.trigger)}>
...
{#if open}
	...
	>
		{#if arrow > 0}
			<span class={stringify(css.arrow)} {...$_arrow} use:$_arrow.action />
		{/if}
		<slot />
	</div>
{/if}
