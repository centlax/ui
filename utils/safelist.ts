...
const safelistByComponent = {
  ...
  input: (colorsAsRegex: any) => [{
    ...
    pattern: new RegExp(`caret-(${colorsAsRegex})-600`)
  }, {
    pattern: new RegExp(`caret-(${colorsAsRegex})-500`),
    variants: ['dark']
  }]
}
...
export const customSafelistExtractor = (prefix: any, content: string, colors: string[], safelistColors: string[]) => {
  const classes = []
  ...
  for (const match of matches) {
    ...
    // @ts-ignore
    const matchClasses = safelistByComponent[name](color).flatMap((group: { variants: any; pattern: { source: string } }) => {
      return ['', ...(group.variants || [])].flatMap(variant => {
        ...
        return matches.map((match: string) => {
          ...
          return colorOptions.map((color: any) => `${variant ? variant + ':' : ''}` + group.pattern.source.replace(match, color))
        }).flat()
      })
    })
