<template>
  <div :class="ui.wrapper" v-bind="$attrs">
    <div v-if="props.label || $slots.label" :class="[ui.label.wrapper]">
      <label :for="inputId" :class="[ui.label.base, required ? ui.label.required : '']">
        {{ label }}
      </label>
      <span v-if="props.hint || $slots.hint" :class="[ui.hint]">
        <slot name="hint" v-bind="{ error, label, name, hint, help }">{{ props.hint }}</slot>
      </span>
    </div>
