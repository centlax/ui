...
import { usePopper, usePortal, type InteractOutsideEvent } from '@melt-ui/svelte/internal/actions';
...
export function createPopover(args?: CreatePopoverProps) {
	...
	const trigger = makeElement(name('trigger'), {
		...
		action: (
			node: HTMLElement,
			custom: HTMLElement
		): MeltActionReturn<PopoverEvents['trigger']> => {
			activeTrigger.set(custom);
